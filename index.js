...
Promise.all(
    ...
    ]).then((values) => 
    {
        ...
        if (terms.length <= imgs.length) {
            imgs.forEach(img => {
                fs.rmSync(__dirname + "/" + img);
            });
        }
        var contentRandIndex = randIndex(terms.length);
        while (result_contain(contentRandIndex)) {
            contentRandIndex = randIndex(terms.length);
        }
        
        var line = terms[contentRandIndex];
